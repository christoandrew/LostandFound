...
<div class="row">
    ...
    <div class="col-lg-12 banner">
        <div class="container">
            <h2>Musical</h2>
            <p>
                Here you can find any Musical Equipment that you lost.
            </p>
        </div>
    </div>
                ...
                <div class="tab-content">
                    <div class="tab-pane active" id="list">
                        <table class="table" id="container" style="background-color: #ffffff">
                            ...
                            <tbody>
                            <?php
                            $getAllMusical = $mysqli->query("SELECT items.*, DATE_FORMAT(items.DatePosted,'%b %d %Y') AS DatePosted FROM items WHERE Category = '13' AND verified=0");

                            if ($getAllMusical) {
                                if (mysqli_num_rows($getAllMusical) > 0) {
                                    while ($rows = mysqli_fetch_array($getAllMusical, MYSQL_ASSOC)) {
                    ...
                    <div class="tab-pane" id="grid" style="">
                        <div class="row">
                            <?php
                            $getAllMusical = $mysqli->query("SELECT * FROM items WHERE Category = '9' AND verified=0");
                            if ($getAllMusical) {
                                if (mysqli_num_rows($getAllMusical) > 0) {
                                    while ($rows = mysqli_fetch_array($getAllMusical, MYSQL_ASSOC)) {
