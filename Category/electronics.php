...
<div class="row">
    ...
    <div class="col-lg-12 banner">
        <div class="container">
            <h2>Electronics</h2>
            <p>
                Here you can find any Electronics that you lost.
            </p>
        </div>
    </div>
                ...
                <div class="tab-content">
                    <div class="tab-pane active" id="list">
                        <table class="table" id="container" style="background-color: #ffffff">
                            ...
                            <tbody>
                            <?php
                            $getAllElectronics = $mysqli->query("SELECT items.*, DATE_FORMAT(items.DatePosted,'%b %d %Y') AS DatePosted FROM items WHERE Category = '6' AND verified=0");

                            if ($getAllElectronics) {
                                if (mysqli_num_rows($getAllElectronics) > 0) {
                                    while ($rows = mysqli_fetch_array($getAllElectronics, MYSQL_ASSOC)) {
                    ...
                    <div class="tab-pane" id="grid" style="">
                        <div class="row">
                            <?php
                            $getAllElectronics = $mysqli->query("SELECT * FROM items WHERE Category = '6' AND verified=0");
                            if ($getAllElectronics) {
                                if (mysqli_num_rows($getAllElectronics) > 0) {
                                    while ($rows = mysqli_fetch_array($getAllElectronics, MYSQL_ASSOC)) {
