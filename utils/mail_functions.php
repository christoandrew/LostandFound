...
function format_reset_email($info, $format)
{
    //set the root
    $root = $_SERVER['DOCUMENT_ROOT'] . '/LostAndFound';;

    //grab the template content
    $template = file_get_contents($root . '/utils/reset_template.' . $format);
...
function format_contact_email($info, $format){
    //set the root
    $root = $_SERVER['DOCUMENT_ROOT'] . '/LostAndFound';;

    //grab the template content
    $template = file_get_contents($root . '/utils/contact_template.' . $format);
...
function send_feedback_email($info){
    //format each email
    $body = format_contact_email($info, 'html');
    //$body_plain_txt = format_reset_email($info, 'txt');
...
//Set who the message is to be sent to
    ...
    $mail->addAddress('lostandfound256@gmail.com', 'Hi Admin');
...
function send_reset_email($info)
{
    //format each email
    $body = format_reset_email($info, 'html');
    //$body_plain_txt = format_reset_email($info, 'txt');
...
//Set who the message is to be sent to
    ...
    $mail->addAddress($info['email'], 'Hi User');
...
function format_verification_email($info, $format)
{
    ...
    //replace all the tags
    $template = preg_replace('/CODE/', $info['code'], $template);
